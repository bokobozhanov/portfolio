...
const animateTransition = (startEl, endEl, imgURL) => {
  ...
  // Make sure panel is snapped into final position and scroll transforms are applied
  forceRecenter();

  // Wait one frame so transforms/layout settle, then measure and animate.
  requestAnimationFrame(() => {
    ...
    // Schedule mover cleanup and reveal panel
...
const createMoverStyle = (step, index, imgURL) => {
  const style = {
    ...
    position: 'fixed', // viewport-based coordinates
    left: step.left,   // numbers from generateMotionPath -> gsap will set px
    top: step.top,
    width: step.width,
    height: step.height,
    clipPath: getClipPathsForDirection(config.clipPathDirection).from,
    zIndex: 9999 + index, // make sure it's above other elements
    backgroundPosition: '50% 50%',
    transformOrigin: 'center center',
  };

  // Optional rotation (keeps CSS property name compatible)
  const rz = gsap.utils.random(-config.rotationRange, config.rotationRange);
  if (rz !== 0) style.rotationZ = rz;
...
gsap.set(panel.querySelector('.panel__img'), {
  ...
  gsap
    .timeline({
	  ...
	  onStart: () => {
        forceRecenter();   // ✅ recenter BEFORE animation starts
